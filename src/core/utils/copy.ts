...
export const copyPackageToStore = async (options: {
  ...
  let npmList: string[] = []
  try {
    // npm-packlist v10 requires a more complete tree structure
    // Create a minimal tree object with required properties
    const tree = {
      path: workingDir,
      package: pkg,
      edgesOut: new Map(), // Required by npm-packlist v10
      children: new Map(), // Required by npm-packlist v10
    }
